...
import { TitleFavorite } from './title-favorite'
import { HiStar } from 'react-icons/hi'
import Link from 'next/link'

export const TitleCard: React.FC<{ data: IMovieData }> = ({ data }) => {
  const title = data.name || data.title || data.original_title || data.original_name || ''
  const src = TMDB_POSTER_PATH_185x278 + data.poster_path || TMDB_POSTER_PATH_PLACEHOLDER
  const year = data.release_date || data.first_air_date || null
  const note = data.vote_average.toFixed(1) || 0

  return (
    <div className="relative">
      <TitleFavorite data={data} />

      <Link href={`/${data.media_type}/${data.id}`}>
        <img src={src} className="w-full aspect-[9/14] rounded-md bg-foreground" alt={title} title={title} />
